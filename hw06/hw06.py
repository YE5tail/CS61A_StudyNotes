    ...
    def vend(self):
        if not self.stock:
            ...
            if self.funds >= self.price:
                ...
                add_on = ''
                if change:
                    add_on = f' and ${change} change'
                self.funds = 0
                self.stock -= 1
                return f'Here is your {self.product}' + add_on + '.'
            else:
                margin = self.price - self.funds
                return f'You must add ${margin} more funds.'
...
class Coin:
    ...
    def worth(self):
        add_value = max(0, (Mint.present_year - self.year - 50))
        return self.cents + add_value
...
def two_list(vals, amounts):
    ...
    """
    # 这是看了一眼别人的答案，豁然开朗了，我可真笨！
    if len(amounts) == 1 and amounts[0] == 0: # 最终的base case
        return Link.empty
    if amounts[0] == 0: # 阶段性的base case， move on
        vals, amounts = vals[1:], amounts[1:]
    amounts[0] -= 1
    return Link(vals[0], two_list(vals, amounts))

"""
# 这是别人的答案，真厉害。
def two_list_alt(vals, amounts):
    if len(amounts) == 0:
        return Link.empty
    if amounts[0] == 0:
...
def two_list(vals, amounts):
    ...
    for i in range(len(vals)):
        for amount in range(amounts[i]):
            ls.append(vals[i])
    return create_link(ls)

def create_link(ls):
    if not ls:
        return Link.empty
    else:
        return Link(ls[0], create_link(ls[1:]))
"""
...
class VirFib():
    ...
    def next(self):
        next_num = self.value + self.previous
        next_fib = VirFib(next_num)
