...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            assert place.ant.can_contain, 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cnt = 0
        while True:
            if check_place.is_hive:
                return None
            elif check_place.bees and (self.min_range <= cnt <= self.max_range):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.place:
            ...
            d_damage = amount + self.damage
        for b in now_place.bees[:]:
            b.reduce_health(d_damage)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_countdown > 0:
            ...
            if self.place.bees:
                victim = random_bee(self.place.bees)
                victim.reduce_health(victim.health)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        if not (other.is_container or self.ant_contained):
            return True
        else:
            return False
        # END Problem 8
...
class TankAnt(ContainerAnt):
    """protects an ant in its place and also
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        its health to 0."""
        # BEGIN Problem 10
        super().add_insect(insect) # super() 指向 Place（inheritance)
