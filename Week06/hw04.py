...
def mul_interval(x, y):
    ...
    # data abstration 就是要把定义和实际运算分开。
    # 违反 abstraction barrier 就是将定义和实际运算混起来，一会儿用selector，一会儿用[]。
    # 在这题中，我们不能假定lower_bound(x)就是
    # x[0]，我们只能用lower_bound()这个‘selector’去把量提取出来。
    # 同理，我们也不能假设return值是一个list，而需要用interval()这个‘constructor’去把该值
    # 变成我们定义的量。否则的话，如果我们改变定义，所有的运算都会发生错误。
...
def sub_interval(x, y):
    ...
    biggest = upper_bound(x) - lower_bound(y)
    return interval(lowest, biggest)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert (lower_bound(y) * upper_bound(y)) > 0, 'Should not divided by 0'
